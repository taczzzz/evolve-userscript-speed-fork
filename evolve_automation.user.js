...
// @downloadURL  https://raw.githubusercontent.com/taczzzz/evolve-userscript-speed-fork/main/evolve_automation.user.js
// @updateURL    https://raw.githubusercontent.com/taczzzz/evolve-userscript-speed-fork/main/evolve_automation.meta.js
// @author       Fafnir
// @author       TMVictor
// @author       Vollch
// @author       schoeggu
...
(function($) {
    ...
    var state = {
        ...
        warnDebug: true,
        warnPreload: true,
        gameSpeedPatchInstalled: false,
        gameSpeedMultiplierApplied: 1,
        gameSpeedRestartTimer: null,
        workerPostMessageOriginal: null,
    ...
    function updateGeneralSettingsContent() {
        ...
        addSettingsSelect(currentNode, "gameSpeedMultiplier", "时间流速", "重设当前页面游戏 worker 的主循环周期。1x 为官方原速，10x 会把 250ms 的主循环压到约 25ms。刷新页面后脚本会重新接管。", [
            {val: "1", label: "1x", hint: "官方原速"},
            {val: "2", label: "2x", hint: "约 125ms 主循环"},
            {val: "5", label: "5x", hint: "约 50ms 主循环"},
            {val: "10", label: "10x", hint: "约 25ms 主循环"},
            {val: "20", label: "20x", hint: "约 13ms 主循环"},
            {val: "50", label: "50x", hint: "约 5ms 主循环"},
            {val: "100", label: "100x", hint: "约 3ms 主循环，极易卡顿或异常"},
    ...
    function getMainVue() {
        let element = win?.$('#topBar')?.[0] ?? win?.document?.querySelector('#topBar');
        return element?.__vue__;
    }

    function getGameSpeedMultiplier() {
        let raw = settings.gameSpeedMultiplier ?? settingsRaw.gameSpeedMultiplier ?? "1";
        let parsed = Number(raw);
        return Number.isFinite(parsed) && parsed >= 1 ? parsed : 1;
    }

    function restartGameLoopForSpeed() {
        let mainVue = getMainVue();
        if (!mainVue?.pause) {
            return false;
        }

        if (state.gameSpeedRestartTimer) {
            win.clearTimeout(state.gameSpeedRestartTimer);
        }

        // The game only stops its worker on the following long loop, so restarting must be delayed.
        if (!mainVue.s.pause) {
            mainVue.pause();
        }

        state.gameSpeedRestartTimer = win.setTimeout(() => {
            let resumedVue = getMainVue();
            if (resumedVue?.pause && resumedVue.s.pause) {
                resumedVue.pause();
            }
            state.gameSpeedRestartTimer = null;
        }, 6000);

        return true;
    }

    function installGameSpeedPatch() {
        if (state.gameSpeedPatchInstalled || !win?.Worker?.prototype?.postMessage) {
            return false;
        }

        state.workerPostMessageOriginal = win.Worker.prototype.postMessage;
        win.Worker.prototype.postMessage = function(message, ...args) {
            if (message?.loop === 'start' && typeof message.period === "number") {
                let multiplier = getGameSpeedMultiplier();
                if (multiplier > 1) {
                    message = {...message, period: Math.max(1, Math.round(message.period / multiplier))};
                }
            }

            return state.workerPostMessageOriginal.call(this, message, ...args);
        };
        state.gameSpeedPatchInstalled = true;
        return true;
    }

    function applyGameSpeed(force = false) {
        let multiplier = getGameSpeedMultiplier();
        if (!state.gameSpeedPatchInstalled || (!force && state.gameSpeedMultiplierApplied === multiplier)) {
            return false;
        }

        state.gameSpeedMultiplierApplied = multiplier;
        restartGameLoopForSpeed();
        return true;
    }
